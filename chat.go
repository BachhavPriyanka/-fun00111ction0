...
func registerOrLogin(conn net.Conn) (string, error) {
	...
	// It will read the client's response
...
func login(conn net.Conn, reader *bufio.Reader, writer *bufio.Writer) (string, error) {
	if _, err := writer.WriteString("Enter your username: "); err != nil {
		return "", err
	}
	...
	if _, err := writer.WriteString("Enter your password: "); err != nil {
		return "", err
	}
	...
	oldPassword, ok := users[username]
	if !ok {
		return "", fmt.Errorf("user %s not found", username)
	}

	if oldPassword == password {
		if _, err := writer.WriteString("Login successful!"); err != nil {
			return "", err
		}
	}
